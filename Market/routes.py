    ...
    if request.method == 'POST':
        ...
        if p_item_object:
            if current_user.can_purchase(p_item_object):
                ...
                db.session.commit()
                flash(f'Congratulations! You purchased the item {p_item_object.name} for {p_item_object.price}$.',category="success")
            else:
                flash(f'Unfortunately you have {p_item_object.price - current_user.budget}$ less credit to purchase {p_item_object.name}',category="danger")
